...
class CalendarWidgetApp:
    ...
    def make_day_cell(self, parent: tk.Frame, day: date, count: int, in_month: bool) -> tk.Frame:
        ...
        if count:
            count_fg = self.theme["error"] if self.is_current_week_day(day) else "#FFFFFF"
        ...
        while current < month_end:
            ...
            if day_events:
                ...
                for event in day_events:
                    ...
                    week_tags = ("week_event",) if self.is_current_week_day(current) else ()
                    ...
                    title_tags = (*base_title_tags, *week_tags, *click_tags)
    ...
    def send_due_notifications(self) -> None:
        ...
        for event in self.events:
            ...
            notify_at = event_start - timedelta(minutes=minutes_before)
            if not notify_at <= now < event_start:
                continue

            notification_key = f"{event.privacy_key}:{event_start.isoformat()}:{minutes_before}"
    ...
    def show_event_notification(self, event: CalendarEvent) -> bool:
        try:
        ...
        event_title = self.display_event_title(event)
        try:
            toast = Notification(
                app_id="Google Calendar Widget",
                title="\uc77c\uc815",
                msg=f"{event_title} \uc774 {self.notification_minutes_before}\ubd84 \ub4a4\uc5d0 \uc608\uc815\ub418\uc5b4 \uc788\uc2b5\ub2c8\ub2e4.",
                duration="short",
            )
            toast.show()
    ...
    def prune_sent_notification_keys(self, now: datetime) -> None:
        active_keys = {
            f"{event.privacy_key}:{event.start.isoformat()}:{self.notification_minutes_before}"
            for event in self.events
